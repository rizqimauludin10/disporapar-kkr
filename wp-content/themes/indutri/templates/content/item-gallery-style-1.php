<?php 
  $gallery_color = get_post_meta( get_the_ID(), 'indutri_gallery_color', true );
         ...
         if($i==1){ ?>
            <div class="image-item" id="<?php echo get_the_ID() ?>">
               <a href="<?php echo esc_url($image_full_src) ?>" class="zoomGallery" data-rel="lightGallery">
                  <span class="icon-expand"><i class="fi flaticon-add"></i></span>
                  <img src="<?php echo esc_url($image_thumb_src) ?>"  class="hidden no-lazyload" alt="<?php the_title_attribute() ?>" />
               </a>
         ...
         <?php }else{ ?>
            <div class="image-item">
               <a href="<?php echo esc_url($image_full_src) ?>" class="zoomGallery hidden" data-rel="lightGallery">
                  <img src="<?php echo esc_url($image_thumb_src) ?>" alt="<?php the_title_attribute() ?>" class="hidden no-lazyload" />
               </a>
            </div>
         <?php }
      endforeach;
