 ...
 */
   ...
   $sidebar_layout_config = indutri_get_option('archive_service_sidebar', 'right-sidebar'); 
   $left_sidebar = indutri_get_option('archive_service_left_sidebar', 'service_sidebar');
   $right_sidebar = indutri_get_option('archive_service_right_sidebar', 'service_sidebar');
   ...
   $columns_lg = indutri_get_option('service_columns_lg', '4');
   $columns_md = indutri_get_option('service_columns_md', '3');
   $columns_sm = indutri_get_option('service_columns_sm', '2');
   $columns_xs = indutri_get_option('service_columns_xs', '1');
          ...
          <div id="wp-content" class="wp-content clearfix">
            <div class="lg-block-grid-<?php echo esc_attr($columns_lg) ?> md-block-grid-<?php echo esc_attr($columns_md) ?> sm-block-grid-<?php echo esc_attr($columns_sm) ?> xs-block-grid-<?php echo esc_attr($columns_xs) ?>">
               <?php while ( have_posts() ) : the_post(); ?>
                  <?php get_template_part( 'templates/content/item', 'service-style-1' ); ?>
               <?php endwhile; ?>  
            </div>
          </div>    
        </div>      
