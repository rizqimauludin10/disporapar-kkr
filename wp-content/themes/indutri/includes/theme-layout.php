...
================================================== */
if ( ! function_exists( 'indutri_base_sidebar' ) ) {
     function indutri_base_sidebar() {
        // VARIABLES
        global $post;

        // DEFAULT SIDEBAR CONFIG
        $default_sidebar_config = indutri_get_option('default_sidebar_config', ''); 
        $default_left_sidebar   = indutri_get_option('default_left_sidebar', ''); 
        $default_right_sidebar  = indutri_get_option('default_right_sidebar', '');
        ...
        if ( is_search() || is_archive() || is_author() || is_category() || is_home() ) {
          $default_sidebar_config = indutri_get_option('archive_sidebar_config', ''); 
          $default_left_sidebar   = indutri_get_option('archive_sidebar_left', '');
          $default_right_sidebar  = indutri_get_option('archive_sidebar_right', ''); 
        }
        ...
        if ( $post && is_singular() ) {
          $sidebar_config = get_post_meta( $post->ID, 'indutri_sidebar_config', true );
          $left_sidebar   = get_post_meta( $post->ID, 'indutri_left_sidebar', true );
          $right_sidebar  = get_post_meta( $post->ID, 'indutri_right_sidebar', true );
        }
...
function indutri_sidebar_global($sidebar_layout_config, $sidebar_left_config, $sidebar_right_config){
...
function indutri_base_layout($template){
  $post_id = indutri_id();
  $sidebar_var           = indutri_base_sidebar();
  ...
  $sidebar_config = indutri_sidebar_global($sidebar_var['config'], $sidebar_var['left'], $sidebar_var['right']);
  extract($sidebar_config);
  
  $page_full_width = get_post_meta($post_id, 'indutri_page_full_width', true);
    ...
    <div class="content-page-wrap">
      <?php do_action( 'indutri_before_main_content' ); ?>
       <div class="main-page-content base-layout row <?php echo esc_attr($page_wrap_class); ?>">
        ...
        <?php do_action( 'indutri_after_main_content' ); ?>
      </div>
    </div>
...
if ( ! function_exists( 'indutri_get_template' ) ) {
    function indutri_get_template( $template ) {
