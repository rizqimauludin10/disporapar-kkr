<?php 
add_filter('add_to_cart_fragments', 'indutri_woocommerce_header_add_to_cart_fragment');
add_action( 'wp_print_scripts', 'indutri_de_script', 100 );

if(!function_exists('indutri_de_script')){
    function indutri_de_script() {
        wp_dequeue_script( 'wc-cart-fragments' );
        return true;
    }
}

if(!function_exists('indutri_woocommerce_query')){
    function indutri_woocommerce_query($type, $post_per_page=-1, $product_cats=''){
...
if(!function_exists('indutri_calc_class_grid')){
    function indutri_calc_class_grid($columns){
        switch ($columns) {
            case '1':
...
if(!function_exists('indutri_woocommerce_header_add_to_cart_fragment')){
    function indutri_woocommerce_header_add_to_cart_fragment( $fragments ) {
        global $woocommerce;
        ob_start();
        indutri_get_cart_contents();
        $fragments['.cart'] = ob_get_clean();
        return $fragments;
    }
}    

if(!function_exists('indutri_get_cart_contents')){
    function indutri_get_cart_contents(){
        global $woocommerce;
        ?>
        <div class="cart dropdown">
            <a class="dropdown-toggle mini-cart" data-toggle="dropdown" aria-expanded="true" role="button" aria-haspopup="true" data-delay="0" href="#" title="<?php esc_html__('View your shopping cart', 'indutri'); ?>">
...
if(!function_exists('indutri_woocommerce_scripts')){
    function indutri_woocommerce_scripts(){
        wp_dequeue_script('wc-add-to-cart');
        wp_register_script( 'wc-add-to-cart', INDUTRI_THEME_URL . '/js/add-to-cart.js' , array( 'jquery' ) );
        wp_enqueue_script('wc-add-to-cart');
    }
    add_action('init','indutri_woocommerce_scripts');    
}